 ...
 *          (or -u)     and assuming it is correct, perform the Suyama PRP test on the cofactor.
 */
...
void print_residues (mpz_t m, int binary, int fnumber, char *name) {
    ...
    if (fnumber < 6) {
        printf ("residue mod F%d:  0x%08lX %10ld", fnumber, res64, res36);
    } else {
        printf ("residue: 0x%016lX |  %11ld  %11ld  %11ld", res64, res36, res36m1, res35m1);
    }
    ...
    if ((binary & 8) == 8) {
        ...
        if (fnumber < 6) {
            printf ("residue mod F%d: %11lo (octal)\n", fnumber, res64);
        } else {
            printf ("residue: 0x%016lX | %012lo %012lo %012lo (octal)\n", res64, res36, res36m1, res35m1);
        }
    }
    ...
    if ((binary & 2) == 2) {
        ...
        printf ("residue mod F%d: ", fnumber);
...
void usage (int verbose) {
    ...
    printf ("Default (mode 1) runs a P%lspin test, followed by a Suyama cofactor test if factors are supplied.\n\n", pe);
    ...
    printf ("    -k            Use currently known prime factors (as of 2012) for the Fermat number being tested.\n");
    ...
    printf ("    -upr file     Bypass the P%lspin test, read the Suyama A residue from the mprime proof file and \n", pe);
    printf ("                  use it to complete the Suyama cofactor test (mode 3 is much quicker to run)\n\n");
    printf ("Some examples:    cofact 5 641                 cofact -t 4 20\n");
    printf ("                  cofact -k 12                 cofact -ku F17.proof 17      (mode 3)\n");
    printf ("\n");
    if (verbose) {
        printf ("----------------------------------------------------------------------------------------------------\n");
        printf ("\n");
    ...
    for (argi = 1; argi < argc; argi++) {
        ...
        if (strncmp(argv[argi], "-", 1) == 0) {     // Combined -abcdvhikmoux flags, processed in that order
            ...
            if (flags != NULL) {
                ...
                if (flags != NULL) { check_proof_res = 1; z++; }
                ...
                if (flags != NULL) { usage (verbose); exit (0); }
                ...
                if (flags != NULL) { use_proof_res = 1; z++; }
...
//      print_mpz (A_proof, 16, "A_proof");
    ...
    if (use_proof_res) {
        ...
        printf ("Skipping the P%lspin test\n\n", pe);
        mpz_set (A, A_proof);
    } else {
        // If not using proof file residue, do the full Pepin and Suyama calculations
        printf ("Testing F%d for primality using the P%lspin test\n", m, pe);
...
//              gwsquare2 (&gwdata, r_gw, r_gw);                        // r_gw = (r_gw ^ 2) mod F      Use this line when using gwnum from mprime v29.8
            ...
            if ((((interim && m > 2) || all_int) && j == 1) || (!interim && j == j_progress_inc)) {
                printf ("Calculating %ld modular squaring iterations from base 3:\n", x);
                if (m > 5 && (interim || all_int)) printf ("Interim residues:                       |      Selfridge - Hurwitz residues\nIteration              mod 2^64 (hex)   |   mod 2^36    mod 2^36-1   mod 2^35-1\n");
            }
        ...
        if (verbose) {
            switch (len) {
                case 1:
                    printf ("P%lspin P%d residue: %016lx\n", pe, m, r_bin[0]);
                    break;
                case 2:
                    printf ("P%lspin P%d residue: %016lx %016lx\n", pe, m, r_bin[1], r_bin[0]);
                    break;
                case 4:
                    printf ("P%lspin P%d residue: %016lx %016lx %016lx %016lx\n", pe, m, r_bin[3], r_bin[2], r_bin[1], r_bin[0]);
                    break;
                default:
                    printf ("P%lspin P%d residue: length = %d words, %016lx %016lx ... %016lx %016lx\n", pe, m, len, r_bin[len-1], r_bin[len-2], r_bin[1], r_bin[0]);
            }
        }
        if (super_verbose || (verbose && m < 12)) {
            printf ("\nP%d == ", m);
        ...
        if (mpz_cmp (P, Fm1) == 0) {
            ...
            for (i = 0; i < k_fact; i++) {              // note, P = 3^2^(2^m - 1) mod F_m = 3^(1/2(F_m - 1)) mod F_m
                mpz_tdiv_r (A, P, kfac[i]);             // A = P (mod p), where p is a prime factor of F_m
                mpz_set_ui (C, 1L);
                mpz_mul_2exp (B, C, x);                 // B = 2^(2^m – 1)
                mpz_sub (Q, kfac[i], C);                // Q = p – 1
                mpz_tdiv_r (C, B, Q);                   // C = 2^(2^m – 1) (mod p – 1)
                mpz_powm (B, three, C, kfac[i]);        // B = 3^C (mod p)
                if (mpz_cmp (A, B) == 0) {
                    printf ("\nP%d == 3^(2^%lu mod ", m, x);
                    ...
                    printf (" mod ");
                    mpz_out_str (stdout, 10, kfac[i]);
                } else {printf ("P%lspin residue has been calculated incorrectly!\n\n", pe); exit (1);}
            }
...
//      gwsquare2 (&gwdata, r_gw, r_gw);        // r_gw = (r_gw ^ 2) mod F      Use this line when using gwnum from mprime v29.8
    ...
    for (i = 0; i < k_fact; i++) {              // note, A == 3^2^2^m mod F_m = 3^(F_m - 1) mod F_m
        mpz_tdiv_r (P, A, kfac[i]);             // P == A (mod p), where p is a prime factor of F_m
        mpz_set_ui (C, 1L);
        mpz_mul_2exp (B, C, exp);               // B = 2^2^m
        mpz_sub (Q, kfac[i], C);                // Q = p – 1
        mpz_tdiv_r (C, B, Q);                   // C == 2^2^m (mod p – 1)
        mpz_powm (B, three, C, kfac[i]);        // B == 3^C (mod p)
        if ((use_proof_res || verbose) && mpz_cmp (P, B) == 0) {
            printf ("A == 3^(2^%lu mod ", exp);
            mpz_out_str (stdout, 10, Q);
            printf (") == ");
            mpz_out_str (stdout, 10, B);
            printf (" mod ");
            ...
            printf ("Problem with proof: Suyama A residue has been calculated incorrectly!\n\n"); 
            exit (1);
        }
    }
    if (k_fact > 0) {
        if (use_proof_res) {symb = "from proof";} else {symb = "also";}
    ...
    if (n_fact > 0) {
        ...
        printf ("Testing the F%d cofactor for primality using the following %d %s factor", m, n_fact, symb);
        ...
        if (n_fact > 1 && digits > 1) {
            if (known_factors) {printf ("Product of known ");} else {printf ("Product of supplied ");}
    ...
    if (n_fact > 0 && digits > 1) {
        ...
        if (mpz_cmp_ui (P, 0L) == 0) {
            ...
            printf ("F%d cofactor is composite, and is %s a prime power\n", m, symb);
            fflush (stdout);
        }
        printf ("\nFactorisation: F%d = ", m);
        for (i = 0; i < n_fact; i++) {
